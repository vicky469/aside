...
test("published artifact path update aborts while queued without overwriting reloaded settings", async () => {
    ...
    const savedPayloads: PersistedPluginData[] = [];
    let saveCount = 0;
    const reloadedData = createSettings({
        publishedPublicArtifactPaths: ["Reloaded/a.html"],
    });
    const harness = createControllerHarness({
        settings: createSettings({
            publishedPublicArtifactPaths: ["Drafts/a.html"],
        }),
        saveData: async (data) => {
            saveCount += 1;
            if (saveCount === 1) {
        ...
        loadData: async () => reloadedData,
    });
    const firstWrite = harness.controller.updatePersistedPluginData((data) => ({
        ...data,
        marker: "first",
    }));
    ...
    const staleUpdate = harness.controller.setPublishedPublicArtifactPaths(
        ["Published/a.html"],
        context,
    );
    abortController.abort();
    const reload = harness.controller.loadSettings();
    releaseFirstSave();
    await firstWrite;
    ...
    assert.deepEqual(harness.getSettings().publishedPublicArtifactPaths, ["Reloaded/a.html"]);

    const currentContext = {
        signal: new AbortController().signal,
        isActive: () => true,
    };
    assert.equal(await harness.controller.setPublishedPublicArtifactPaths(
        ["Current/a.html"],
        currentContext,
    ), true);
    assert.equal(saveCount, 2);
    assert.deepEqual(harness.getSettings().publishedPublicArtifactPaths, ["Current/a.html"]);
    assert.deepEqual(savedPayloads.at(-1)?.publishedPublicArtifactPaths, ["Current/a.html"]);
});

test("settings reload waits for an active event persistence tail before installing new state", async () => {
    ...
    const oldEpochWrite = harness.controller.updatePersistedPluginData((data) => ({
        ...data,
        marker: "old epoch",
    }));
    await saveStarted;
    let reloadSettled = false;
    const reload = harness.controller.loadSettings().then(() => {
        reloadSettled = true;
    });
    await new Promise<void>((resolve) => setImmediate(resolve));
    ...
    await oldEpochWrite;
    await reload;
    assert.deepEqual(harness.controller.readPersistedPluginData(), reloadedData);
    assert.deepEqual(harness.getSettings().publishedPublicArtifactPaths, ["Reloaded/a.html"]);
});

test("stale published artifact persistence failure resolves as aborted before reload", async () => {
    let saveStartedResolve = () => {};
    const saveStarted = new Promise<void>((resolve) => {
        saveStartedResolve = resolve;
    });
    ...
    const harness = createControllerHarness({
        ...
        saveData: async () => {
            saveStartedResolve();
            await saveGate;
            throw new Error("stale published path save failed");
        },
    });
    const abortController = new AbortController();
    const context = {
