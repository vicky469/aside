...
test("ensurePrivatePublishIndexMarkdown renders published URLs after path and upgrades old managed sections", () => {
	const entries: Array<PrivatePublishIndexEntry & { publishedUrl: string | null }> = [
		{
			path: "roadmap.md",
			publishedUrl: "https://publish.example.com/public/roadmap",
			type: "file",
			status: "published",
			permissionSource: "auth.md",
			lastPublishedAt: "2026-07-26T08:00:00.000Z",
	...
	assert.equal(
		ensurePrivatePublishIndexMarkdown(oldManagedMarkdown, entries),
		[
			"# Published Index",
			"",
			"<!-- Aside publish index -->",
			"| path | published_url | type | status | permission_source | last_published_at |",
			"| --- | --- | --- | --- | --- | --- |",
			"| draft.md |  | file | unpublished | auth.md |  |",
			"| roadmap.md | https://publish.example.com/public/roadmap | file | published | auth.md | 2026-07-26T08:00:00.000Z |",
			"<!-- /Aside publish index -->",
			"",
		].join("\n"),
	);
