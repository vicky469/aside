...
test("agent actor registry resolves actors by directive mention", () => {
    ...
    assert.equal(getAgentActorByDirectiveMention("@CuRsOr")?.id, "cursor");
...
test("agent actor registry exposes every local CLI as a peer supported actor", () => {
    assert.deepEqual(getSupportedAgentActors().map((actor) => actor.id), ["codex", "claude", "cursor", "gemini", "deepseek"]);
    assert.equal(formatSupportedAgentDirectives("or"), "@codex, @claude, @cursor, @gemini, or @deepseek");
});
...
test("agent actor registry keeps unsupported notices generic when all known actors are supported", () => {
    ...
    assert.equal(resolveUnsupportedAgentNotice(["gemini"]), "This build currently supports @codex, @claude, @cursor, @gemini, and @deepseek only.");
});
...
test("agent actor registry normalizes any-vs-supported targets separately", () => {
    ...
    assert.equal(normalizeAnyAgentTarget("CURSOR"), "cursor");
    assert.equal(normalizeSupportedAgentTarget("CURSOR"), "cursor");
