...
Why does SideNote2 poll `pollFocusedSyncedSideNoteEvents()` every 750ms, and can index/sidebar refreshes be driven by Obsidian events instead?
...
- files visible in SideNote2 sidebar leaves
...
event -> classify affected files -> replay synced events if needed -> load/cache comments
      ...
      -> refresh or schedule SideNote2 index.md
```
...
| --- | --- | --- | --- |
| Plugin load/layout ready | visible files + aggregate | all open SideNote2 views | scheduled |
...
| Open SideNote2 index | aggregate before open | index sidebar view | immediate before open |
...
4. If the changed file set is unknown, refresh open SideNote2 views and schedule the aggregate index.
...
- Switching back to `SideNote2 index.md` updates index sidebar state without polling.
