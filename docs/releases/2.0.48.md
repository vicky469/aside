...
- `Move to another file` now targets markdown files directly, opens the destination note after the move while reusing an existing tab when possible, and nested child notes can now be re-parented locally with the same drag handle used for page-note ordering.

## Notes

- This release does not change the stored Aside thread format. The move-flow updates stay within the existing note-backed data model.
