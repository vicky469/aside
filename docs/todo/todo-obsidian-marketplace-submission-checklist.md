...
- [x] The plugin source is public on GitHub at `vicky469/aside`.
...
- [x] Plugin id is `aside`, which does not contain `obsidian`.
- [x] `aside` is not already present in Obsidian's current `community-plugins.json` as of 2026-04-14.
...
- [ ] Open the submission PR with title `Add plugin: Aside`.
