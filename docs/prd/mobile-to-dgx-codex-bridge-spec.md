# Mobile To DGX Codex Bridge Spec
...
- [mobile-to-macos-codex-bridge-spec.md](mobile-to-macos-codex-bridge-spec.md)
...
2. SideNote2 routes that run to an NVIDIA DGX Spark
3. the DGX launches the same local `codex` CLI family that desktop SideNote2 uses today
4. the remote DGX returns reply text only
...
- the DGX is compute, not workspace ownership
...
This DGX-specific spec exists to answer:

- how to use one specific DGX Spark as the remote execution owner
- what the remote DGX is allowed to do
...
The user-facing mobile-to-DGX route should not try to reproduce that behavior.

Instead:

- desktop local = workspace-aware local execution
- mobile to DGX = reply-only remote execution
...
Obsidian mobile
  ...
  -> HTTPS bridge running on the DGX Spark
  -> local codex CLI on that same DGX Spark
...
- no extra SSH hop
- DGX is used purely as compute
- no need to mirror the vault onto the DGX
...
Obsidian mobile
  -> SideNote2 remote bridge client
  -> HTTPS bridge / relay
  -> SSH target session on the DGX Spark
  -> local codex CLI on the DGX Spark
```

Use the SSH deployment only if:

- the bridge cannot run on the DGX directly
...
The remote DGX must not need:

- the DGX's absolute vault path
...
```ts
interface MobileDgxBridgeMetadata {
  vaultName: string;
  notePath: string;
...
The remote DGX should receive that final prompt and generate a reply from it.
...
3. SideNote2 sends final prompt text to the remote DGX
4. remote DGX returns reply text only
...
The DGX bridge must return explicit user-safe errors for these cases.
...
- bridge is healthy
- but `codex` is not installed or not signed in on the DGX
...
- the phone cannot reach the DGX bridge
...
### Phase 1: Prepare the DGX

1. Install and verify the local `codex` CLI on the DGX.
2. Verify:
...
3. Make sure the DGX can stay reachable over LAN, VPN, or Tailscale.

### Phase 2: Run the bridge on the DGX

1. Start a small HTTPS service on the DGX.
...
3. Make the bridge launch `codex` locally on the DGX.
...
2. Enter the DGX bridge HTTPS URL.
...
4. no Codex on the DGX
   Expected:
   - clear runtime failure notice

## Security Rules

- never let the remote DGX edit the repo or vault for this user-facing mobile path
...
- mobile-triggered `@codex` replies through a specific DGX
- safe canonical write-back into the original markdown thread
- a user-facing mobile path that does not require repo access

## Recommendation

Keep the mobile-to-DGX route reply-only.

If a future advanced power-user mode wants workspace-aware remote editing on a DGX, that should be a separate non-default spec with stronger safety and mapping rules, not part of the normal mobile user-facing path.
