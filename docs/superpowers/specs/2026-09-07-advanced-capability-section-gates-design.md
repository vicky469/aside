...
Verified on 2026-09-08 without installing, publishing, releasing, or pushing:

- `npm run build` exited 0 with 1,626/1,626 compiled tests and 176/176 direct-source tests passing, followed by clean ESLint, typecheck, Obsidian compliance, production bundle, bundle-size, and integrated release-artifact checks.
- The full regression suite covers default-off migration, section order and disabled detail/search visibility, discovery and actionability, saved routing, script-oriented Generate and retry denial, ordinary agent retry and cancellation preservation, startup vault-event replay, external settings reload batching, active-run preservation, and script lifecycle/retry races.
...
- Public artifact inspection found no `main.js.map`, `sourceMappingURL`, `sourcesContent`, raw TypeScript/TSX/JSX source, secret-bearing content, private-key marker, test fixture, local-only file, or local absolute path. The artifacts were `main.js` (717,606 bytes), `manifest.json` (349 bytes), and `styles.css` (97,579 bytes). The manifest identifies `aside` / `Aside` version `2.0.103`, minimum Obsidian `1.12.7`.
- `main.js` is an expected ignored build product; `manifest.json` and `styles.css` are tracked and remained unchanged. `git diff main...HEAD --check` and the post-code-commit worktree check were clean before this verification record was updated.
- A targeted audit of production source and current guides, excluding historical release notes and historical design records, found no contradictory current default-on or Experimental-settings language. The only production `featureFlags` references intentionally detect and remove legacy persisted data.
