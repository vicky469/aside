...
- `node --test '/Users/example/Obsidian/lean-startup/🛠️ scripts/tests/english-to-chinese.test.mjs'` passed 28 of 28 tests. These tests cover the four-worker ceiling, ordered results, child-process failure paths, batching limits, and bilingual formatting. Two CLI-level signal regressions additionally prove that parent `SIGTERM` reaps a stubborn translator process group, including its descendant, and that the script completes main-operation cleanup before re-raising the original signal. Inspection confirmed the main flow still re-reads the source and performs one final atomic rename only after every batch succeeds.
...
- Direct live child-process counting remains unverified. The first monitor attempted `pgrep` after the short worker window and was denied process-list access; a replacement 100 ms `ps` monitor required sandbox escalation and never started before its approval was interrupted. No second translation was dispatched for that monitor. The focused 28-test suite still proves a maximum of four injected workers, ordered output, and process-group cleanup on parent termination, but this does not replace direct live-process-count evidence.
