...
- [ ] A live four-batch smoke directly observes that active translator child processes never exceed four.
...
- `npm test` passed with 1,200 compiled TypeScript tests and 97 maintained `.mjs` tests; both groups reported zero failures.
...
- `node --test '/Users/example/Obsidian/lean-startup/🛠️ scripts/tests/english-to-chinese.test.mjs'` passed 29 tests with zero failures; one Windows-only descendant-process test was skipped on macOS. The 30-test suite covers the four-worker ceiling, ordered results, child-process failure paths, batching limits, bilingual formatting, and cancellation before atomic commit. CLI-level signal regressions prove that parent `SIGTERM` reaps a stubborn translator process group, including its descendant, and that the script completes main-operation cleanup before re-raising the original signal. Inspection confirmed the main flow still re-reads the source and performs one final atomic rename only after every batch succeeds.
- Three simulated Windows runtime regressions prove that disposal, timeout, and output overflow terminate the complete script process tree through shell-free `taskkill /T /F`; a POSIX regression proves direct-child cancellation leaves output streams unpaused.
...
- One installed-plugin timing probe used a disposable page thread and an 800 ms no-network vault script. The probe wrapped the installed plugin's `workspaceViewController.refreshCommentViews`, measured from the start of saved-entry routing until the first refresh promise resolved, and then queried the rendered output card. This one-off observation completed in 81.9 ms with one blank output entry, the `Script` label, and the existing spinner; it is evidence for the approximate target, not a benchmark. The completed run reused the same output entry id, left exactly two thread entries, replaced the blank body with the final script result, and removed the spinner.
- The no-network probe stayed responsive to a second Obsidian CLI evaluation while execution was pending. Its disposable note/thread was deleted through Obsidian; the temporary `.mjs` script was moved to trash with the vault adapter; visible thread/sidecar state and test-only script-run records were removed.
- A second installed-plugin smoke invoked the registered `/english-to-chinese` script with real Codex subprocesses on a disposable 19-paragraph note, producing four logical batches. Calling the saved-entry router without awaiting its returned promise took 0.000 ms in-app and the CLI kickoff returned in 104.2 ms wall time. The first refresh was called at 31.0 ms and completed at 56.9 ms with a queued run, one blank output entry, one `Script` label, and one spinner.
- The real script run reached `succeeded` 31.518 seconds after its recorded `startedAt`. The final run reused output id `1a79fce1-6fe7-4297-b9ec-fbd3db0854e7`, kept exactly two thread entries, reported `translated 19 paragraph(s)`, and removed the spinner. Structural validation found 19 bilingual blocks, 19 hidden markers, 18 blank separators, and no blank line inside any English/Chinese pair.
- Direct live child-process counting remains unverified. The first monitor attempted `pgrep` after the short worker window and was denied process-list access; a replacement 100 ms `ps` monitor required sandbox escalation and never started before its approval was interrupted. No second translation was dispatched for that monitor. The focused 30-test suite still proves a maximum of four injected workers, ordered output, POSIX process-group cleanup on parent termination, and the Windows process-tree cancellation paths, but this does not replace direct live-process-count evidence.
- Both real-smoke notes were moved to trash through Obsidian. The smoke run record was removed, the plugin reported zero remaining test threads/runs, the Aside sidecar search found no test ids, and a final direct `ps` scan found no matching translation process. No production defect was demonstrated by the completed real translation.
