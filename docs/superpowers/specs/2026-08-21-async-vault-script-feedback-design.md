...
- Direct live child-process counting remains unverified. The first monitor attempted `pgrep` after the short worker window and was denied process-list access; a replacement 100 ms `ps` monitor required sandbox escalation and never started before its approval was interrupted. No second translation was dispatched for that monitor. The focused 26-test suite still proves a maximum of four injected workers and ordered output, but this does not replace direct live-process evidence.
