...
- [x] **Step 3: Implement the shared header presentation**
...
- [x] **Step 1: Write failing deferred-persistence tests**
...
- [x] **Step 3: Implement one mutation transaction helper**
...
- [x] **Step 1: Write the failing coverage tests**
...
- [x] **Step 3: Implement the coverage gate**

Add `hasUnprocessedSnapshotCoverage(coveredWatermarks)` to `SideNoteSyncEventStore`. Compare each covered clock with the current processor watermark. A compacted watermark only describes event-log retention; it does not prove that this processor applied the snapshot. In hydration, skip merging into an existing sidecar when the snapshot has no unseen coverage. Missing sidecars still hydrate for recovery.

- [x] **Step 4: Make sync-state writes atomic**
...
- [x] **Step 1: Write the failing pure reconciliation test**
...
- [x] **Step 3: Implement stable-ID reconciliation**
...
- [x] **Step 4: Apply reconciliation at canonical boundaries**
...
- [x] **Step 3: Inspect shipped artifacts**
...
- [x] **Step 4: Sync and compare `lean-startup`**

Run: `node scripts/install-built-plugin.mjs --vault <vault-path>`, reload Aside, and compare SHA-256 hashes for the three shipped assets.

- [x] **Step 5: Verify and repair the reported thread through the installed build**
