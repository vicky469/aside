...
**Goal:** Prevent post-unload agent dispatch, persist a visible failure card for interrupted runs, and expose `+` on vault-script reply cards.

**Architecture:** `CommentAgentController` remains the owner of agent lifecycle and restart reconciliation. It gains one disposed-state gate shared by saved-entry, retry, queue, and execution paths, while restart reconciliation uses the existing canonical reply commit. `sidebarPersistedComment.ts` removes its script-only action exception and continues through the existing add-entry adapter.
...
- [ ] **Step 1: Write the failing late-callback test**
...
- [ ] **Step 3: Implement bounded interruption recovery**
...
- [ ] **Step 4: Run the restart tests and verify GREEN**
...
### Task 3: Give script replies the shared add action
...
- [ ] **Step 3: Remove the author-specific exception**

Derive `showAddEntryAction` only from the shared redirect/deleted/thread-deleted policy already used by other entries. Keep `renderAddEntryButton` and `startAppendEntryDraft(comment.id, ...)` unchanged so insertion still targets the clicked reply.
...
- [ ] **Step 3: Review privacy and artifacts**
