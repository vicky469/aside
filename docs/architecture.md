...
- `src/core/*` handles storage, anchors, sync policy, text helpers, derived metadata augmentation, editor highlight ranges, and index builders such as `allCommentsNote.ts` and `thoughtTrail.ts`.
- `src/ui/*` handles the sidebar view, the index list / thought-trail mode toggle, draft and persisted comment cards, interaction helpers, editor helpers, settings, and modals.
...
| Sidebar or index sidebar shows wrong grouping, mode, or visibility | `src/ui/views/sidebarCommentSections.ts`, `src/ui/views/SideNote2View.ts`, `src/core/derived/thoughtTrail.ts`, `src/commentManager.ts` |
...
| Index note or thought trail is stale or wrong | `src/index/AggregateCommentIndex.ts`, `src/core/derived/allCommentsNote.ts`, `src/core/derived/thoughtTrail.ts`, `src/control/commentPersistenceController.ts`, `src/cache/ParsedNoteCache.ts` |
...
- The index surfaces are derived views: the note is built by `allCommentsNote.ts`, and the index sidebar can render either the sectioned list or the thought trail.
