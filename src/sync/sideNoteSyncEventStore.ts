...
export class SideNoteSyncEventStore {
    ...
    public async appendExternalEvents(events: readonly SideNoteSyncEvent[]): Promise<number> {
        ...
        const state = this.readState();
        const importedClocksByDevice = new Map<string, number[]>();
        let importedCount = 0;
        for (const event of normalizedEvents) {
            const compactedClock = state.compactedWatermarks[event.deviceId] ?? 0;
            if (event.logicalClock <= compactedClock) {
                continue;
            }

            const deviceLog = state.deviceLogs[event.deviceId] ?? {
                lastClock: compactedClock,
                events: [],
            };
            if (
                deviceLog.events.some((existing) =>
                    existing.eventId === event.eventId || existing.logicalClock === event.logicalClock)
            ) {
                state.deviceLogs[event.deviceId] = deviceLog;
                continue;
            }

            const expectedClock = Math.max(deviceLog.lastClock, compactedClock) + 1;
            if (event.logicalClock !== expectedClock) {
                state.deviceLogs[event.deviceId] = deviceLog;
                continue;
            }

            deviceLog.events = deviceLog.events.concat(cloneEvent(event)).sort(compareEvents);
            deviceLog.lastClock = Math.max(deviceLog.lastClock, event.logicalClock);
            state.deviceLogs[event.deviceId] = deviceLog;
            importedCount += 1;
            const clocks = importedClocksByDevice.get(event.deviceId) ?? [];
            clocks.push(event.logicalClock);
            importedClocksByDevice.set(event.deviceId, clocks);
        }
        ...
        for (const [deviceId, clocks] of importedClocksByDevice.entries()) {
            const selfWatermarks = {
                ...(state.processedWatermarks[deviceId] ?? {}),
            };
            selfWatermarks[deviceId] = advanceContiguousWatermark(
                Math.max(
                    selfWatermarks[deviceId] ?? 0,
                    state.compactedWatermarks[deviceId] ?? 0,
                ),
                clocks,
            );
            state.processedWatermarks[deviceId] = selfWatermarks;
        }
