...
function renderCommentsRoute(): string {
	return [
		"import { privatePublishManifest } from \"../../../../src/_aside/private-publish-data.js\";",
		"import { createPublishedCommentEvent, getAsideSessionIdentity, getPublishedComments, resolvePrivatePublishPermission } from \"../../../../src/_aside/private-publish-runtime.js\";",
		"",
		"export async function onRequestGet(context) {",
		...
		"\t}",
		"\tconst result = await createPublishedCommentEvent(context.env, { path, body, identity });",
		"\treturn result.ok",
...
function renderPrivatePublishRuntimeModule(): string {
	return [
		...
		"\t}",
		"\ttry {",
		"\t\tconst result = await db.prepare(\"SELECT event_id, path, op, payload_json, author_provider, author_identity, author_display_name, created_at FROM aside_comment_events WHERE path = ? ORDER BY created_at ASC, event_id ASC\").bind(normalizedPath).all();",
		"\t\treturn {",
		"\t\t\tok: true,",
		"\t\t\tcomments: (Array.isArray(result?.results) ? result.results : []).map(commentRowToPublishedComment).filter(Boolean),",
		"\t\t};",
		"\t} catch {",
		...
		"\ttry {",
		"\t\tawait db.prepare(\"INSERT INTO aside_comment_events (event_id, path, op, payload_json, author_provider, author_identity, author_display_name, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)\").bind(",
		"\t\t\teventId,",
		"\t\t\tnormalizedPath,",
		"\t\t\t\"createThread\",",
		"\t\t\tpayloadJson,",
		...
		"\treturn {",
		"\t\tok: true,",
		"\t\tcomment: commentRowToPublishedComment({",
		"\t\t\tevent_id: eventId,",
		"\t\t\tpath: normalizedPath,",
		"\t\t\top: \"createThread\",",
		"\t\t\tpayload_json: payloadJson,",
		"\t\t\tauthor_provider: identity.provider,",
		"\t\t\tauthor_identity: identity.identifier,",
		"\t\t\tauthor_display_name: identity.name ?? null,",
		...
		"\tif (!row || typeof row !== \"object\" || row.op !== \"createThread\") {",
		"\t\treturn null;",
		"\t}",
